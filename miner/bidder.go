...
func (b *Bidder) mainLoop() {
	...
	for chainHeadEvent := range b.chainHeadCh {
		...
		nextBlockNumber := chainHeadEvent.Block.Number().Int64() + 1

		betterBidBefore := bidutil.BidBetterBefore(chainHeadEvent.Block.Header(), b.chain.Config().Parlia.Period, b.delayLeftOver,
		...
		for {
			...
			case <-second.C:
			case <-third.C:
				b.bid(b.getBestWork(nextBlockNumber))
			case <-b.exitCh:
				return
			default:
			}
